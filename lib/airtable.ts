...
export const CLUB_MEMBERS_TABLE =
  process.env.AIRTABLE_CLUB_MEMBERS_TABLE ?? "ClubMembers";
export const ACCESS_REQUESTS_TABLE =
  process.env.AIRTABLE_ACCESS_REQUESTS_TABLE ?? "AccessRequests";
...
export function noteCall(table?: string) {
  ...
  console.log(
    `AIRTABLE CALL +1 table=${table ?? "<unknown>"} total=${airtableStats.calls}`,
  );
}
...
export async function cachedFirstPage(
  table: string,
  selectOptions: any = {},
  ttlSeconds = 15,
) {
